...
int main(int argc, char* argv[])
{
    ...
    while(!done)
    {
        ...
        switch(schedChoice)
        {
            ...
            case 2:
				...
				//////////////////////////////
                if(curTime==0||procList[procIdx].isDone){
                    for(int i=0;i<numProc;i++){
                        if(procList[i].totalTimeNeeded<min&&!procList[i].isDone&&procList[i].startTime<=curTime){
                            min=procList[i].totalTimeNeeded;
                            procIdx=i;
                        }
                    }
                }
            ...
            case 3:
				...
				//////////////////////////////
                for(int i=0;i<numProc;i++){
                    if(procList[i].totalTimeNeeded-procList[i].timeScheduled<min&&!procList[i].isDone&&procList[i].startTime<=curTime){
                        min=procList[i].totalTimeNeeded-procList[i].timeScheduled;
                        procIdx=i;
                    }
                }
            ...
            case 4:
                ...
                if(curTime==0||procList[procIdx].isDone){
                    for(int i=0;i<numProc;i++){
                        nexthrr=double(curTime-procList[i].startTime-procList[i].timeScheduled+procList[i].totalTimeNeeded)/procList[i].totalTimeNeeded;
                        if(nexthrr>currhrr&&!procList[i].isDone&&procList[i].startTime<=curTime){
                            currhrr=nexthrr;
                            procIdx=i;
                        }
                    }
    ...
    for (auto &p :procList){
        p.turnaroundTime=p.timeFinished-p.startTime;
        p.normalizedTurnaroundTime=1.0*p.turnaroundTime/p.timeScheduled;
        averageNormTurnTime+=p.normalizedTurnaroundTime;
    }
